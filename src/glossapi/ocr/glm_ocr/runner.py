"""GLM-OCR runner — in-process MLX execution with CLI subprocess and stub fallback.

GLM-OCR (https://huggingface.co/mlx-community/GLM-OCR-4bit) is a compact
0.5B-parameter vision-language model fine-tuned for document OCR.  The
MLX-community quantised variant runs efficiently on Apple Silicon.
...
def _run_inproc(
    ...
    *,
    ...
    LOGGER.info("Loading GLM-OCR MLX model from %s", model_path)
    ...
    for file_idx, (name, pdf_path) in enumerate(zip(file_list, resolved_paths), 1):
        ...
        if not pdf_path.exists():
            LOGGER.warning("GLM-OCR MLX: PDF not found: %s", pdf_path)
            results[stem] = {"page_count": 0}
            continue
        LOGGER.info(
            "GLM-OCR MLX: processing file %d/%d — %s",
        ...
        try:
            ...
            LOGGER.error(
                "GLM-OCR MLX in-process failed for %s: %s", pdf_path.name, exc,
            )
            raise
    return results
...
def _resolve_mlx_cli_script() -> Path:
    ...
    Priority: ``GLOSSAPI_GLMOCR_MLX_SCRIPT`` env var > package-embedded script.
    """
    env_script = os.environ.get("GLOSSAPI_GLMOCR_MLX_SCRIPT", "").strip()
    if env_script:
        p = Path(env_script)
        if p.exists():
            return p
        LOGGER.warning(
            "GLOSSAPI_GLMOCR_MLX_SCRIPT=%s does not exist; using package script",
            env_script,
        )
...
def _run_mlx_cli(
    ...
    *,
    ...
    """Invoke the GLM-OCR MLX CLI as a subprocess."""
    ...
    env = os.environ.copy()
    LOGGER.info("Running GLM-OCR MLX CLI: %s", " ".join(cmd))
...
def run_for_files(
    ...
    **_: Any,
    ...
    """Run GLM-OCR for the provided files.
    ...
    if model_dir is None:
        ...
        model_dir = resolve_weights_dir("glm-ocr-mlx")
    ...
    if missing:
        LOGGER.warning(
            "GLM-OCR: %d input file(s) not found in candidate roots; OCR may be incomplete.",
            len(missing),
        )
    ...
    if (
        ...
        try:
            LOGGER.info("GLM-OCR: using in-process MLX execution")
            return _run_inproc(
                resolved_paths,
                file_list,
                out_root,
            ...
            LOGGER.warning(
                "GLM-OCR in-process MLX execution failed (%s); trying next strategy",
                exc,
            )
    ...
    if use_cli and platform.system() == "Darwin" and mlx_script_path.exists():
        ...
        try:
            ...
            if not use_stub:
                raise
            LOGGER.warning(
                "GLM-OCR MLX CLI failed (%s); falling back to stub output", exc,
            )
    elif use_cli and platform.system() != "Darwin":
        msg = "GLM-OCR CLI requested on non-macOS"
        if not use_stub:
    ...
    if not use_stub:
        raise RuntimeError(
            "GLM-OCR: no execution strategy available "
            "(in-process, MLX CLI, and stub are all disabled or failed)"
        )

    LOGGER.info("GLM-OCR: using stub output for %d file(s)", len(file_list))
    results = {}
    for name, pdf_path in zip(file_list, resolved_paths):
        stem = Path(name).stem
        md_path = md_dir / f"{stem}.md"
