...
- `MINERU_MIN_BATCH_INFERENCE_SIZE` (default `384` in MinerU, overridden to `50` by GlossAPI setup): number of pages processed per inference chunk. MinerU runs the full model stack (Layout, MFD, MFR, OCR) per chunk before moving to the next. Smaller values reduce the number of formula crops processed in a single MFR pass, limiting per-pass state accumulation. Larger values reduce the number of Layout/MFD warm-up invocations. Tune based on formula density and available memory.
