...
- `GLOSSAPI_MINERU_MFR_BATCH_SIZE` (default `32` on MPS, MinerU default elsewhere): number of formula crops fed to UniMerNet per GPU batch. MinerU's own default (~64) causes unified-memory pressure on Apple Silicon, leading to MFR Predict throughput collapsing mid-stage (e.g. 25 it/s → <3 it/s). Values in the range `16`–`32` give more consistent sustained throughput. Set to `0` to leave MinerU's default untouched. Injected into the resolved `magic-pdf.json` at runtime via `formula-config.mfr_batch_size`.
