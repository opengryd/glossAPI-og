...
- `MINERU_MIN_BATCH_INFERENCE_SIZE` (default `384` in MinerU, overridden to `500` by GlossAPI setup): number of pages processed per inference chunk. MinerU runs the full model stack (Layout, MFD, MFR, OCR) per chunk. Set above your document's page count to avoid mid-document splits and the associated Layout/MFD re-warmup overhead.
