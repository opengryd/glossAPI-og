...
- `GLOSSAPI_MINERU_MPS_HIGH_WATERMARK_RATIO` (default `0.7`): PyTorch MPS allocator high-watermark multiplier injected as `PYTORCH_MPS_HIGH_WATERMARK_RATIO` into the `magic-pdf` subprocess on macOS/MPS. PyTorch's default is `1.7` (allocate up to 1.7× the recommended memory budget before GC). Reducing this to `0.7` fires GC at 70% of the budget, producing many small GC pauses instead of one large stall that leaves the GPU idle mid-stage (most visible in MFR Predict). Set to `0` to disable and revert to PyTorch defaults.
- `GLOSSAPI_MINERU_MPS_LOW_WATERMARK_RATIO` (default `0.5`): PyTorch MPS allocator low-watermark multiplier (`PYTORCH_MPS_LOW_WATERMARK_RATIO`). After GC fires, the allocator drains memory down to this fraction of the recommended budget. Must be less than the high-watermark value (PyTorch enforces `low < high`).
- `GLOSSAPI_MINERU_MFR_BATCH_SIZE` (default `16` on MPS, MinerU default elsewhere): number of formula crops fed to UniMerNet per GPU batch. MinerU's own default (~64) causes unified-memory pressure on Apple Silicon, leading to MFR Predict throughput collapsing mid-stage (e.g. 25 it/s → <3 it/s). Values in the range `8`–`16` give more consistent sustained throughput. Set to `0` to leave MinerU's default untouched. Injected into the resolved `magic-pdf.json` at runtime via `formula-config.mfr_batch_size`.
- `GLOSSAPI_MINERU_OCR_REC_BATCH_SIZE` (default `6` on MPS, MinerU default elsewhere): number of text crops decoded per OCR-rec batch. Controls `ocr-config.rec_batch_num` in the resolved config. Smaller values reduce peak memory and produce more even OCR-rec throughput; MinerU's own default is already modest (~6) so this mainly matters if you raise it for CUDA.
- `GLOSSAPI_MINERU_LAYOUT_BATCH_SIZE`: batch size for layout detection (`layout-config.batch_size`). No MPS default is applied; set explicitly to override MinerU's default. A one-shot CNN, so less sensitive to batch size than MFR or OCR-rec.
